  ...
  getList(isReplaceList = false, isLoadMore = false) {
    ...
    app.http(query, parmas).then(data => {
      data.list.forEach(item => {
        ...
        // orderType += item.enoughStatus === 1 && item.orderStatus === "090001" || item.orderStatus === "090002" || item.enoughStatus === 1 && item.orderStatus === "090004" && item.oando === "down"?"A(可出货)/":""

        orderType +=   item.enoughStatus === 1 ? item.orderStatus === "090001" || item.orderStatus === "090002" ? "A(可出货)/" : item.orderStatus === "090004" && item.oando === "down"?"A(可出货)/":'':''

        // orderType += item.enoughStatus === 2 && item.orderStatus === "090001" || item.orderStatus === "090002" || item.enoughStatus === 2 && item.orderStatus === "090004" && item.oando === "down" ? "P(库存不足)/" : ""
        orderType += item.enoughStatus === 2 ? item.orderStatus === "090001" || item.orderStatus === "090002" ? "P(库存不足)/" : item.orderStatus === "090004" && item.oando === "down" ? "P(库存不足)/" : '' : ''
