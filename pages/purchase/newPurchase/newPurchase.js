 ...
 Page({
       ...
       data.list.forEach(item => {
         if (item.dftStatus === "1") {
           this.setData({
             ...
             deliveryAddressId: item.tableKey
           })
         }
       })
     })
   ...
   formSubmit(e) {
     ...
     if (!info.buyer&&!this.data.isAutoAssign) {
       app.showToast("请选择采购员")
       return
     }
     if (!info.storehouse&&!this.data.isAutoAssign) {
       app.showToast("请选择仓库")
       return
     }
     if (!info.receiveAddress&&!this.data.isAutoAssign) {
       app.showToast("请选择收货地址")
       return
     }
     if (!info.receiver&&!this.data.isAutoAssign) {
       app.showToast("请填写收货人姓名")
       return
     }
     if (!info.phoneNumber&&!this.data.isAutoAssign) {
     ...
     if (this.data.isAutoAssign) { 
       ...
       app.http("allotOrderPrev", {
        ...
        wx.navigateTo({
          url: '/pages/purchase/viewSplitOrder/viewSplitOrder'
        })
       })
